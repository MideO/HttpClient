...
sealed abstract class AbstractHttpRequest[T: ClassTag](Method: HttpMethod,
                                                       Url: String,
                                                       headers: Map[String, Object],
                                                       entity: Option[T],
  ...
  private def doSend[M: ClassTag]: () => Try[HttpResponse[M]] = () => {
  ...
  private def responseBody(implicit connection: HttpURLConnection): Seq[Byte] = {
    connection match {
      ...
      case it if it.getErrorStream != null => Source.fromInputStream(it.getErrorStream).mkString.getBytes
